...
**📋 Planning Scope Note**: This specification describes the complete cross-platform todo list system (Android, Windows, and Backend). The current planning cycle (**Feature 001**) covers **backend API implementation only**.

**Separate planning cycles** will address:
- Android client implementation (Kotlin, Jetpack Compose, Room)
- Windows client implementation (C# WinUI 3, Entity Framework Core)

See `tasks.md` for the complete backend task breakdown (47 tasks, T001-T047).
...
3. **Given** user is viewing the task list, **When** user selects delete/remove on a task, **Then** the task disappears from the list immediately and deletion syncs to backend
...
- Cross-device task sharing is a client-side UX concern. Backend is token-agnostic and accepts any valid UUID v4 device token. Clients may implement token sharing via QR code scanning, manual copy/paste, cloud backup codes, or device pairing—backend design supports all approaches by simply filtering tasks by the provided token. Token sharing mechanism is deferred to client implementation phase.
